...
*/
function saveCode() {
  var fileName = window.prompt('¿Cúal podría ser el nombre del archivo?', 'ROVOFIC_Blockly_File ')
...
function load(event) {
  ...
  reader.onloadend = function(event) {
    ...
    if (target.readyState == 2) {
      ...
      if (count && confirm('¿Desea remplazar bloques existentes?\n"Cancel" will merge.')) {
...
function discard() {
  var count = Blockly.mainWorkspace.getAllBlocks().length;
  if (count < 2 || window.confirm('¿Desea borrar ' + count + ' blocks?')) {
    Blockly.mainWorkspace.clear();
    renderContent();
  }
}
...
function uploadClick() {
    ...
    uploadCode(code, function(status, errorInfo) {
        if (status == 200) {
            ...
            alert("Hubo un error al cargar el programa: " + errorInfo);
        }
    });
}
...
function resetClick() {
    ...
    uploadCode(code, function(status, errorInfo) {
        if (status != 200) {
            alert("Error reseteanado el programa: " + errorInfo);
        }
    });
}
